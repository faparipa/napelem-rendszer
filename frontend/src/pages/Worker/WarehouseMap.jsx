...
const WarehouseMap = ({ highlightedSlotId }) => {
  const { allSlots, parts, fetchMapStatus, fetchParts, updateStock } =
    useWorkerStore();
  ...
  const handleUpdateStock = async (amount) => {
    ...
    try {
      await updateStock(editingSlot.id, partToUse, amount);
      setEditingSlot(null);
      setQty(0);
    } catch (err) {
      // Store kezeli a hibaüzenetet
    }
  };
