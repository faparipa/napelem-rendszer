...
const WarehouseExpansion = () => {
  const expandWarehouse = useWarehouseStore((state) => state.expandWarehouse);
  const [config, setConfig] = useState({
    addRows: 0,
    addCols: 0,
    levels: 3,
  ...
  const handleExpand = async () => {
    if (
      !window.confirm(
        `Biztosan hozzáadsz ${config.addRows} új sort és ${config.addCols} új oszlopot?`
      )
    )
      return;
    await expandWarehouse(config);
  };
